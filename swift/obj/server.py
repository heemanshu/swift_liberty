...
class ObjectController(BaseStorageServer):
	...
	start_time = time.time()
        fw.write('Started Object PUT at ' + str(start_time) + "\n")
        device, partition, account, container, obj, policy = \
            get_name_and_placement(request, 5, 5, True)
        req_timestamp = valid_timestamp(request)
        ...
        try:
    	    time_a = time.time()
            fw.write('Entering get_diskfile at ' + str(time_a) + "\n")
            disk_file = self.get_diskfile(
                device, partition, account, container, obj,
                policy=policy, frag_index=frag_index)
	    time_a_end = time.time()
	    fw.write('Returned from get_diskfile at ' + str(time_a_end) + " with time taken as " + str((time_a_end - time_a)*1000) + "\n")
        except DiskFileDeviceUnavailable:
            return HTTPInsufficientStorage(drive=device, request=request)
        try:
    	    time_b = time.time()
            fw.write('Entering read_metadata at ' + str(time_b) + "\n")
            orig_metadata = disk_file.read_metadata()
	    time_b_end = time.time()
	    fw.write('Returned from read_metadata at ' + str(time_b_end) + " with time taken as " + str((time_b_end - time_b)*1000) + "\n")
	...
	time_b_2 = time.time()
        fw.write('Entering md5 at ' + str(time_b_2) + "\n")
        etag = md5()
	time_b_2_end = time.time()
	fw.write('Returned from md5 at ' + str(time_b_2_end) + " with time taken as " + str((time_b_2_end - time_b_2)*1000) + "\n")
        elapsed_time = 0
        try:
		    ...
		    time_b_3 = time.time()
        	    fw.write('Entering send_hundred_continue_response at ' + str(time_b_3) + "\n")
                    request.environ['wsgi.input'].\
                        send_hundred_continue_response()
		    time_b_3_end = time.time()
		    fw.write('Returned from send_hundred_cotninue_response at ' + str(time_b_3_end) + " with time taken as " + str((time_b_3_end - time_b_3)*1000) + "\n")
		...
		time_c = time.time()
            	fw.write('Entering writer.commit at ' + str(time_c) + "\n")
                writer.commit(request.timestamp)
	    	time_c_end = time.time()
	    	fw.write('Returned from writer.commit at ' + str(time_c_end) + " with time taken as " + str((time_c_end - time_c)*1000) + "\n")
	...
	time_d = time.time()
        fw.write('Entering container_update at ' + str(time_d) + "\n")
        self.container_update(
            'PUT', account, container, obj, request,
            update_headers,
            device, policy)
	time_d_end = time.time()
	fw.write('Returned from container_update at ' + str(time_d_end) + " with time taken as " + str((time_d_end - time_d)*1000) + "\n")

	end_time = time.time()
        fw.write('Leaving Object PUT at ' + str(end_time) + " with time taken as " + str((end_time - start_time)*1000) + "\n")
	fw.close()
